...
/**
 * Created by LHZ on 2016/10/25.
 */
public class CustomGrok extends Grok {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(CustomGrok.class);
    /**
    ...
    /** only use in parse.grok discovery. */
    ...
    /**
     * Create a {@code Grok} instance with the given parse.grok patterns file.
     *
     * @param  grokPatternPath : Path to the pattern file
     * @return Grok
     * @throws GrokException runtime expt
    ...
    /**
     * Add custom pattern to parse.grok in the runtime.
     *
     ...
     *
     * @param file : Path of the parse.grok pattern
     * @throws GrokException runtime expt
     */
    public void addPatternFromFile(String file) throws GrokException {
        ...
        if(f.isDirectory()){
            ...
            for(File tfile:files){
                addPatternFromFile(tfile.getAbsolutePath());
            }
            return;
        }
        if (!f.canRead()) {
    ...
    /**
     * Original parse.grok pattern used to compile to the named regex.
     *
